 ...
 * `generate` returns only a waveform, and the waveform's length depends on the
...
export interface TransformersModule {
   ...
   * unreported rather than being reported wrongly.
   */
  readonly StoppingCriteria?: new () => StoppingCriteriaLike;
  /**
   * Optional so an older build, or a test double, still satisfies the type.
   * Required members here break `loadModule`, which is public API — that was
   * an oversight when this was added, not a decision.
   */
  readonly AutoConfig?: {
    from_pretrained(
      modelId: string,
